...
export default function SearchMap({ profilePageExists, profiles, center, zoom }) {

  let circleValue;
  if (profilePageExists) {
    circleValue = (<Circle
      center={{ lat: profiles[0].home_lat, lng: profiles[0].home_long }}
      radius={100}
      text="" />);
  }
  ...
  return (
    ...
    <div className='map'>
      <GoogleMapReact
        ...
        center={
          profiles[0] ? (
            { lat: profiles[0].home_lat, lng: profiles[0].home_long }
          ) : (
              center
            )
        }
      ...
      >
        {profiles.map((mapProfile, index) => {
          if (!profilePageExists &&
            mapProfile.user_id.toString() !== localStorage.getItem('loggedInUsersId')) {
            return (
              <ProfilePins
                key={index}
